...
import { EmptyArray, enforceMonomorphism } from './utils';
...
export function patchChildren(
    ...
    if (process.env.NODE_ENV !== 'production') {
        for (const vnode of [...c1, ...c2]) {
            if (!isNull(vnode) && !isString(vnode)) {
                enforceMonomorphism(vnode);
            }
        }
    }
