...
export function createWeakMultiMap<K extends object, V extends object>(): WeakMultiMap<K, V> {
    // For the time being at least, it is not intended to leak this to prod. Shipping memory leaks to
    // browsers that don't support WeakRefs in prod mode could be bad.
    assertNotProd();
