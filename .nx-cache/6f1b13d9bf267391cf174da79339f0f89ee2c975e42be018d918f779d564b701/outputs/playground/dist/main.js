...
function validateClassAttr(vnode, elm, renderer) {
  ...
  const scopedToken = getScopeTokenClass(owner); // Classnames for scoped CSS are added directly to the DOM during rendering,
  ...
  if (scopedToken) {
    if (!isUndefined$1(className)) {
      ...
      classMap = Object.assign(Object.assign({}, classMap), {
        [scopedToken]: true
      });
    }
  }
...
function tmpl$1($api, $cmp, $slotset, $ctx) {
  ...
  const {_m0, _m1} = $ctx;
  return [api_text("Counter: " + api_dynamic_text($cmp.counter)), api_element("button", {
    key: 0,
    on: {
      "click": _m0 || ($ctx._m0 = api_bind($cmp.decrement))
    }
  }, [api_text("-")]), api_element("button", {
    key: 1,
    on: {
      "click": _m1 || ($ctx._m1 = api_bind($cmp.increment))
    }
  }, [api_text("+")])];
  /*LWC compiler v2.22.0*/
}
...
function tmpl($api, $cmp, $slotset, $ctx) {
  ...
  return [api_static_fragment($fragment1(), 1), api_custom_element("x-counter", _xCounter, stc0)];
  /*LWC compiler v2.22.0*/
}
